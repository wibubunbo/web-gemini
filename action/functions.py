...
def fill(bid: str, value: str):
    ...
    def do(force: bool):
        if demo_mode != "off":
            ...
            elem.clear(force=force, timeout=1500)
            elem.type(value, delay=delay, timeout=0)  # no timeout
        else:
            elem.fill(value, force=force, timeout=1500)
...
def check(bid: str):
    ...
    def do(force: bool):
        elem.check(force=force, timeout=1500)
...
def uncheck(bid: str):
    ...
    def do(force: bool):
        elem.uncheck(force=force, timeout=1500)
...
def select_option(bid: str, options: str | list[str]):
    ...
    def do(force: bool):
        elem.select_option(options, force=force, timeout=1500)
...
def click(
    ...
    def do(force: bool):
        elem.click(button=button, modifiers=modifiers, force=force, timeout=1500)
...
def hover(bid: str):
    ...
    def do(force: bool):
        elem.hover(force=force, timeout=1500)
...
def press(bid: str, key_comb: str):
    ...
    elem = get_elem_by_bid(page, bid, demo_mode != "off")
    add_demo_mode_effects(page, elem, bid, demo_mode=demo_mode, move_cursor=False)
    elem.press(key_comb, timeout=1500)
